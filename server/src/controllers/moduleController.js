  ...
  try {
    ...
    if (module.visibility === 'private') {
      ...
      if (!req.userId) {
        return res
          .status(401)
          .json({ success: false, message: 'Please log in to view this module' });
      }

      const isOwner = module.owner._id.toString() === req.userId.toString();
      const isCollaborator = module.collaborators.some(
      ...
      if (!isOwner && !isCollaborator) {
        return res
          .status(403)
          .json({ success: false, message: 'Not authorized to view this module' });
      }
    }
...
exports.updateModule = async (req, res) => {
  try {
    ...
    const collaborator = module.collaborators.find(
    ...
    if (!isOwner && !isEditor) {
      return res
        .status(403)
        .json({ success: false, message: 'Not authorized to update this module' });
    }
...
exports.deleteModule = async (req, res) => {
  try {
    ...
    if (module.owner.toString() !== req.userId.toString()) {
      return res
        .status(403)
        .json({ success: false, message: 'Not authorized to delete this module' });
    }
