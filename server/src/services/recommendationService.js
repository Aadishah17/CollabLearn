...
const recommendationService = {
  ...
  async getPersonalizedRecommendations(userId, limit = 20) {
    ...
    // Double-check filter
    const safeFilteredSkills = availableSkills.filter(
      (skill) => skill.user && skill.user._id.toString() !== userId.toString()
    );
    ...
    finalRecommendations = finalRecommendations.filter(
      (rec) => rec.user && rec.user._id.toString() !== userId.toString()
    );
...
const analyzeUserProfile = async (userId, userSkills, userBookings, userPosts) => {
  ...
  if (offeringSkills.length > 0) {
    ...
    profile.skillLevel = Object.keys(levelCounts).reduce(
  ...
  if (userBookings.length > 0) {
    profile.learningIntensity =
      userBookings.length /
      Math.max(1, (Date.now() - new Date(userBookings[0].createdAt)) / (1000 * 60 * 60 * 24 * 30));
    ...
    profile.pricePreferences.prefersFree = userBookings.some(
      (b) => (b.skill?.offering?.price || 0) === 0
    );
  }
  ...
  if (userPosts.length > 0) {
    profile.socialEngagement =
      userPosts.reduce(
        (sum, post) => sum + (post.stats?.likes || 0) + (post.stats?.comments || 0),
        0
      ) / userPosts.length;
  }
...
const calculateAdvancedRecommendationScore = async (skill, userProfile) => {
  ...
  if (instructor) {
    ...
    if (instructor.totalSessions)
      qualityScore += Math.min(4, Math.log10(instructor.totalSessions + 1));
  ...
  if (skillPrice === 0 && userProfile.pricePreferences.prefersFree) {
    ...
    skillPrice >= userProfile.pricePreferences.min &&
