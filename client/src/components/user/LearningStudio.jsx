...
const AudioScript = ({ data }) => {
  ...
  if (!paragraphs.length)
    return <p className="text-zinc-400 text-sm">No audio script generated.</p>;
    ...
    <div className="studio-result-panel">
      ...
      <div className="audio-player">
        <div className="audio-player-controls">
          {!playing ? (
            <button className="audio-play-btn" onClick={playAudio} disabled={!audioSupported}>
              <Play size={20} fill="white" />
            </button>
          ) : (
            <button className="audio-play-btn" onClick={pauseAudio}>
              <Pause size={20} fill="white" />
            </button>
          )}
          <button className="studio-nav-btn" onClick={stopAudio} disabled={!playing && !paused}>
            <Square size={14} />
          </button>
        </div>
        ...
        <div className="audio-progress-area">
          ...
          <p className="text-[11px] text-zinc-500 mt-1">
            {data.duration} •{' '}
            {playing ? 'Playing...' : paused ? 'Paused' : progress === 100 ? 'Finished' : 'Ready'}
          </p>
          {!audioSupported && (
            <p className="text-[11px] text-amber-300 mt-1">
              Audio playback is not supported in this browser. The transcript is still available.
            </p>
          )}
        </div>
      </div>
      ...
      <div className="mt-4">
        <div className="flex items-center justify-between mb-2">
          ...
          <button className="studio-nav-btn" onClick={copyAll}>
        ...
        <div className="space-y-3 max-h-[300px] overflow-y-auto pr-1">
          {paragraphs.map((p, i) => (
            <p
              key={i}
              className={`text-sm leading-relaxed transition-colors duration-300 ${currentPara === i ? 'text-white font-medium' : 'text-zinc-400'}`}
            >
              {p}
            </p>
          ))}
...
const SlidesViewer = ({ data }) => {
  ...
  const slideContent = (isFs) => (
    <div
      className={`slide-presentation ${isFs ? 'slide-fullscreen' : ''}`}
      style={{ background: bg }}
      ref={containerRef}
    >
      {isFs && (
        <button className="slide-exit-fs" onClick={() => setFullscreen(false)}>
      ...
      <div className="slide-content-area">
        ...
        {slide.footer && (
          <p className={`slide-footer ${isFs ? 'text-base' : 'text-xs'}`}>{slide.footer}</p>
        )}
      </div>
  ...
  return (
    <div className="studio-result-panel">
      <div className="flex items-center justify-between mb-3">
        ...
        <button className="studio-nav-btn" onClick={() => setFullscreen(true)}>
  ...
  return (
    <div className="studio-result-panel">
      <div className="flex items-center justify-between mb-3">
        ...
        <button className="studio-nav-btn" onClick={doShuffle}>
  ...
  if (current >= questions.length) {
    return (
      <div className="studio-result-panel text-center">
        ...
        <div className="quiz-score-bar mt-3">
          <div
...
const MindMapView = ({ data }) => {
  ...
  const svgW = 900,
    svgH = Math.max(500, branches.length * 120);
  const cx = svgW / 2,
    cy = svgH / 2;
      ...
      <div className="overflow-x-auto">
        <svg width={svgW} height={svgH} viewBox={`0 0 ${svgW} ${svgH}`} className="mind-map-svg">
          ...
          <rect
            ...
            strokeWidth={2}
          />
          <text x={cx} y={cy + 6} textAnchor="middle" fill="#fca5a5" fontSize={16} fontWeight={700}>
            {data.root}
          </text>
  ...
  return (
    <div className="studio-result-panel">
      <div className="flex items-center justify-between mb-4">
        ...
        <button className="studio-nav-btn" onClick={exportNotes}>
        ...
        <div key={i} className="mb-4">
          ...
          <ul className="space-y-1">
            {(section.bullets || []).map((bullet, bi) => (
              <li key={bi} className="text-sm text-zinc-300 flex items-start gap-2">
                <span className="text-red-400 mt-1">•</span> {bullet}
              </li>
            ))}
          </ul>
        </div>
...
const ReportView = ({ data }) => {
  ...
  return (
    <div className="studio-result-panel">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-lg font-semibold">{data.title}</h3>
        <button className="studio-nav-btn" onClick={exportReport}>
          <Download size={14} /> Download .md
        </button>
      </div>
      {data.executive_summary && (
        <p className="text-sm text-zinc-300 mb-4 italic border-l-2 border-red-500/40 pl-3">
          {data.executive_summary}
        </p>
      )}
      {data.sections.map((section, i) => (
...
const InfographicView = ({ data }) => {
  if (!data?.stats?.length)
    return <p className="text-zinc-400 text-sm">No infographic data generated.</p>;
  return (
    <div className="studio-result-panel">
        ...
        <div className="space-y-2 mt-4">
          ...
          {data.facts.map((fact, i) => (
            <div key={i} className="infographic-fact">
              <div
                className="infographic-fact-bar"
                style={{ width: `${Math.min(100, 60 + i * 8)}%` }}
              />
...
const DataTableView = ({ data }) => {
  ...
  if (!data?.columns?.length || !data?.rows?.length)
    return <p className="text-zinc-400 text-sm">No data table generated.</p>;
  ...
  const exportCSV = () => {
    ...
    const csv = [
      ...
      ...sortedRows.map((r) => (Array.isArray(r) ? r : []).map(escape).join(',')),
    ].join('\n');
    downloadFile(
      csv,
      `${(data.title || 'table').replace(/\s+/g, '-').toLowerCase()}.csv`,
      'text/csv'
    );
  ...
  return (
    <div className="studio-result-panel">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-lg font-semibold">{data.title}</h3>
        <button className="studio-nav-btn" onClick={exportCSV}>
              ...
              {data.columns.map((col, i) => (
                <th key={i} onClick={() => handleSort(i)} className="cursor-pointer select-none">
                  <span className="inline-flex items-center gap-1">
                    ...
                    <ArrowUpDown
                      size={12}
                      className={sortCol === i ? 'text-red-400' : 'text-zinc-600'}
                    />
                  </span>
                </th>
              ))}
  ...
  return (
    <div className="glass-panel p-6">
      <div className="flex items-center justify-between mb-5">
        <div>
          <p className="text-[11px] uppercase tracking-widest text-red-400 font-semibold mb-1">
            Studio
          </p>
          <h2 className="text-xl font-semibold">Learning Studio</h2>
        </div>
        {resultTool && (
          <button className="studio-nav-btn" onClick={handleClose}>
            <X size={14} /> Close
          </button>
        )}
      </div>
      ...
      {loading && !result && (
        <div className="flex items-center justify-center gap-3 py-8">
          ...
          <p className="text-sm text-zinc-400">
            Generating {STUDIO_TOOLS.find((t) => t.id === activeTool)?.label || 'content'}...
          </p>
        </div>
      )}
