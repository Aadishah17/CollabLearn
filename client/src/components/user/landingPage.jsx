...
export default function CollabLearnLanding() {
  ...
  return (
    <div className="dashboard-shell glass-page min-h-screen overflow-x-hidden text-slate-100 selection:bg-red-500/30">
      <a href="#main-content" className="sr-only focus:not-sr-only focus:absolute focus:left-4 focus:top-4 focus:z-[60] focus:rounded-lg focus:bg-red-600 focus:px-4 focus:py-2 focus:text-sm focus:font-semibold focus:text-white">
      ...
      <SocialProofSection />
      <FaqSection />
      <TeachSection hasSession={hasSession} userRole={userRole} isSuperAdmin={isSuperAdmin} publicEntry={publicEntry} navigate={navigate} />
      </main>
        ...
        <div className="mx-auto flex max-w-7xl flex-col gap-6 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-wrap gap-5 text-sm font-semibold text-zinc-300">
            {footerLinks.map((link) =>
              link.href ? (
                <a key={link.label} href={link.href} className="transition-colors hover:text-red-300">
                  {link.label}
                </a>
              ) : (
                <Link key={link.label} to={link.to} className="transition-colors hover:text-red-300">
                  {link.label}
                </Link>
              ),
            )}
