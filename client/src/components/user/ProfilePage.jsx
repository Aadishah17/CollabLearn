...
export default function ProfilePage() {
  ...
  const deleteOfferingSkill = async (skillId) => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        `${API_URL}/api/skills/offering/${skillId}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const deleteSeekingSkill = async (skillId) => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        `${API_URL}/api/skills/seeking/${skillId}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
    ...
    return (
      <div className="min-h-screen bg-black pt-20 flex items-center justify-center">
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-lg text-gray-400">Loading profile...</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-black pt-20">
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        <div className="bg-black border border-white rounded-2xl shadow-sm p-8 mb-6 transform transition-all duration-300 hover:shadow-md">
          <div className="flex items-start justify-between">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  {profileData.isPremium ? (
                    ...
                    <span className="inline-flex items-center px-3 py-1 bg-gray-800 text-gray-300 text-sm font-medium rounded-full">
                      Free
                    </span>
                  )}
                </div>
                ...
                <div className="flex items-center gap-4 mb-3 text-gray-600">
                  {profileData.joinDate && (
                    <div className="flex items-center gap-1 transition-colors duration-200 hover:text-red-600">
                      <Calendar size={16} />
                  ...
                  <div className="flex items-center">
                    {[...Array(5)].map((_, i) => (
                      <Star
                        key={i}
                        size={16}
                        className={`${i < Math.floor(profileData.rating.average)
                ...
                <div className="flex items-center gap-6 text-sm">
                  {profileData.email && (
                    <div className="flex items-center gap-2 text-red-600 transition-all duration-200 hover:text-red-700">
                      <Mail size={16} />
            ...
            <div className="flex flex-col items-end gap-4">
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 bg-black border border-white text-white rounded-lg hover:bg-gray-900 transition-all duration-200 hover:shadow-md cursor-pointer"
              >
                <Edit size={18} />
        ...
        {/* Tabs */}
        <div className="bg-black border border-white rounded-2xl shadow-sm mb-6">
          <div className="flex border-b border-gray-700">
            {tabs.map((tab) => (
              <button
                key={tab.id}
                onClick={() => setActiveTab(tab.id)}
                className={`flex-1 px-6 py-4 text-sm font-medium transition-all duration-200 cursor-pointer ${activeTab === tab.id
        ...
        {activeTab === "skills" && (
          <div className="space-y-8">
            {/* Skills I Teach */}
            <div className="bg-black border border-white rounded-2xl shadow-sm p-6 transform transition-all duration-300 hover:shadow-md">
              <div className="flex items-center justify-between mb-6">
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-all duration-200 hover:shadow-lg transform hover:scale-105 cursor-pointer"
                >
                  <Plus size={18} />
              ...
              {profileData.skillsOffering.length > 0 ? (
                <div className="flex flex-wrap gap-3">
                  {profileData.skillsOffering.map((skill, index) => (
                    <div
                      key={skill._id}
                      className="border border-white rounded-xl p-4 bg-black transition-all duration-300 hover:shadow-md hover:border-red-500 animate-slide-in min-w-[280px] flex-shrink-0"
                      ...
                      <div className="flex items-start justify-between mb-3">
                        ...
                        <div className="flex items-center gap-2 ml-2">
                          <button
                            ...
                            className="p-1.5 hover:bg-gray-800 rounded-lg transition-colors duration-200 cursor-pointer"
                          >
        ...
        {activeTab === "reviews" && (
          <div className="bg-black border border-white rounded-2xl shadow-sm p-8 text-center">
      ...
      {showAddSkillModal && (
        <div className="fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-50">
          <div className="bg-black border border-white rounded-lg p-6 w-full max-w-md mx-4">
            ...
            <form onSubmit={handleAddSkill} className="space-y-4">
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-2 bg-black border border-gray-600 text-white rounded-lg focus:ring-2 focus:ring-red-600 focus:border-red-600"
                  required
                >
              ...
              <div className="flex gap-3 pt-4">
                <button
                  type="button"
                  onClick={() => setShowAddSkillModal(false)}
                  className="flex-1 px-4 py-2 border border-gray-600 text-gray-300 rounded-lg hover:bg-gray-800 transition-colors duration-200 cursor-pointer"
                >
                  Cancel
                </button>
                <button
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-50">
          <div className="bg-black border border-white rounded-2xl p-6 w-full max-w-md mx-4 animate-fade-in">
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 px-4 py-2 border border-white text-white rounded-lg hover:bg-gray-800 transition-colors duration-200 cursor-pointer"
              >
                Cancel
              </button>
              <button
