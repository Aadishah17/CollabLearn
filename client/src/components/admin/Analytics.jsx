...
export default function AnalyticsDashboard() {
  ...
  const dataCards = [
    ...
    },
    {
      ...
      footer: `${instructorPercent}% of users`,
      footerColor: 'text-gray-500',
      footerIcon: <span className="w-4"></span>,
    },
  ];
      ...
      <div className="pt-24 max-w-7xl mx-auto px-6 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
          {dataCards.map((card, index) => (
            <div
              key={index}
              className="p-6 rounded-xl shadow-lg bg-white border border-gray-200 flex flex-col justify-between"
            >
              <div className="flex items-center justify-between mb-4">
                ...
                <div className={`p-3 rounded-full ${card.color} bg-gray-100`}>{card.icon}</div>
              </div>
              <div
                className={`flex items-center text-sm font-semibold pt-3 border-t border-gray-100 ${card.footerColor}`}
              >
                {card.footerIcon}
