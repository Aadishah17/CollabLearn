    ...
    <div className="h-64 pt-6">
      ...
      <div className="flex justify-center mt-4 space-x-6 text-sm py-5">
        <div className="flex items-center">
          <span className="w-3 h-3 bg-indigo-200 mr-2 border"></span>Registered
        </div>
        <div className="flex items-center">
          <span className="w-3 h-3 bg-indigo-600 mr-2"></span>Active
        </div>
      </div>
    </div>
  );
      ...
      <main className="pt-24 max-w-7xl mx-auto px-6 py-12">
        <header className="mb-12">
          ...
          <p className={`mt-2 ${subtleText}`}>
            The central control panel showing key performance indicators, operational metrics, and
            user analytics.
          </p>
        </header>
            ...
            >
              <div className="flex items-start justify-between mb-4">
                <div className="flex-grow">
                  ...
                  <p className="text-3xl font-extrabold mt-1 text-black">{card.value}</p>
                </div>
                {/* Icon with a neutral gray background circle */}
                <div
                  className={`p-2 rounded-full ${card.iconColor || 'text-indigo-600'} bg-gray-100 flex-shrink-0`}
                >
                  {card.icon}
                </div>
              </div>
              ...
              {/* Card Footer/Detail */}
              <div
                className={`flex items-center text-xs font-semibold pt-3 border-t border-gray-100 ${card.footerColor}`}
              >
                {card.footerIcon}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className={`lg:col-span-2 p-8 rounded-xl shadow-lg bg-white border border-gray-200`}>
            <MonthlyUserChart
              data={summaryData.monthlyData.length > 0 ? summaryData.monthlyData : monthlyData}
            />
          </div>

          {/* User Role Breakdown Section */}
          <div
            className={`p-8 rounded-xl shadow-lg bg-white border border-gray-200 flex flex-col justify-between`}
          >
            ...
            <div className="flex flex-col flex-grow items-center justify-around space-y-4">
              ...
              <div className="text-center p-4 border border-indigo-200 rounded-lg w-full hover:shadow-md transition-shadow">
                ...
                <p className="text-5xl font-extrabold text-indigo-500 my-2">
                  {summaryData.instructors}
                </p>
              ...
              <div className="text-center p-4 border border-purple-200 rounded-lg w-full hover:shadow-md transition-shadow">
                ...
                <p className="text-5xl font-extrabold text-purple-500 my-2">
                  {summaryData.learners}
                </p>
