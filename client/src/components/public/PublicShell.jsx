...
export function PublicShell({ eyebrow, title, copy, chips = [], cta, children }) {
  return (
    <div className="dashboard-shell glass-page min-h-screen overflow-x-hidden text-zinc-100">
      <a href="#public-main-content" className="sr-only focus:not-sr-only focus:absolute focus:left-4 focus:top-4 focus:z-[60] focus:rounded-lg focus:bg-red-600 focus:px-4 focus:py-2 focus:text-sm focus:font-semibold focus:text-white">
      ...
      <main id="public-main-content" className="px-4 pb-16 pt-32 sm:px-6">
        <div className="mx-auto flex w-full max-w-7xl flex-col gap-6">
          <section className="surface-card surface-card-shimmer hero-stage hero-beam relative overflow-hidden p-7 md:p-8">
            <div className="aurora-orb aurora-orb-warm left-[-8%] top-0 h-44 w-44" aria-hidden="true" />
            <div className="aurora-orb aurora-orb-cool right-[3%] top-10 h-52 w-52" aria-hidden="true" />
            ...
            <div className="relative grid gap-8 lg:grid-cols-[1.08fr_0.92fr] lg:items-end">
              <div>
                ...
                <p className="mt-5 max-w-2xl text-base leading-7 text-zinc-300 md:text-lg">{copy}</p>
                {chips.length ? (
  ...
  return (
    <div className="surface-card p-6 md:p-7" role="status" aria-live="polite">
      <div className="space-y-4 animate-pulse">
        ...
        <p className="pt-1 text-sm font-semibold uppercase tracking-[0.28em] text-zinc-500">{label}</p>
      </div>
    </div>
  );
}
