  ...
  return (
    ...
    >
      {state.loading ? (
        ...
        <>
          {state.warning ? (
            <PublicNotice title="Using a fallback fixture" copy={`The public career endpoint was unavailable or empty. ${state.warning}`} />
          ) : null}
          <section className="grid gap-6 lg:grid-cols-[1.05fr_0.95fr]">
            <div className="surface-card p-6 md:p-7">
              <div className="flex flex-wrap items-center gap-2">
                <span className="glass-chip border-white/15 bg-white/[0.04] text-zinc-200">{track.roleTitle || 'Career track'}</span>
                {(track.tags || []).slice(0, 2).map((tag) => (
                  <span key={tag} className="glass-chip border-white/15 bg-white/[0.04] text-zinc-200">
                    {tag}
                  </span>
                ))}
              </div>
              <h2 className="mt-5 text-3xl font-black tracking-tight text-white md:text-4xl">{track.title}</h2>
              <p className="mt-4 max-w-3xl text-base leading-7 text-zinc-300">{track.summary || track.roleSummary}</p>
              <div className="mt-6 grid gap-3 sm:grid-cols-2">
                <PublicNotice title="Role summary" copy={track.roleSummary || 'No role summary provided.'} />
                <PublicNotice title="Hiring advice" copy={track.hiringAdvice || 'No hiring advice provided.'} />
              </div>
            </div>
            <div className="grid gap-4 self-start">
              <div className="surface-card p-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-[0.24em] text-zinc-500">Skill stack</p>
                    <div className="mt-3 flex flex-wrap gap-2">
                      {linkedSkills.length ? linkedSkills.map((item) => (
                        <span key={typeof item === 'string' ? item : item?.name || item?.title} className="glass-chip border-white/10 bg-white/[0.035] text-zinc-200">
              ...
              <div className="surface-card p-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-[0.24em] text-zinc-500">Supporting assets</p>
                    <div className="mt-3 flex flex-wrap gap-2">
                      {[...linkedCourses, ...linkedModules].length ? [...linkedCourses, ...linkedModules].map((item) => (
                        <span key={typeof item === 'string' ? item : item?.name || item?.title} className="glass-chip border-white/10 bg-white/[0.035] text-zinc-200">
          ...
          <section className="grid gap-4 lg:grid-cols-3">
            ...
            <PublicNotice title="Next step" copy={track.cta?.label || 'Continue into the track from the public listing.'} />
          </section>
        </>
      )}
    </PublicShell>
