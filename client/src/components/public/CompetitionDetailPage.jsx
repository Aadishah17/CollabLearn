        ...
        <>
          {state.warning ? (
            <PublicNotice
              ...
              copy={`The public competition endpoint was unavailable or empty. ${state.warning}`}
            />
          ) : null}
            ...
            <div className="surface-card p-6 md:p-7">
              ...
              <p className="mt-4 max-w-3xl text-base leading-7 text-zinc-300">
                {competition.summary ||
                  ...
                  'Guest-facing competition detail page.'}
              </p>
              <div className="mt-6 grid gap-3 sm:grid-cols-2">
                <PublicNotice
                  title="Timeline"
                  copy={`${formatPublicDate(competition.timing?.start)} to ${formatPublicDate(competition.timing?.end)}`}
                />
                <PublicNotice
                  title="Source"
                  copy={competition.sourceUrl || 'Public competition feed'}
                />
              </div>
            </div>
          ...
          <section className="grid gap-4 lg:grid-cols-3">
            ...
            <PublicNotice
              title="Next step"
              copy={competition.cta?.label || 'Enter the competition from the public listing.'}
            />
          </section>
        </>
      )}
